...
This repository contains the source code for the research on solving the 2D Burgers' equation using Physics-Informed Neural Networks (PINNs). The main goal is to perform hyperparameter optimization to find the best model configurations. 
...
2. **Create an experiment configuration:**
   Copy the template `configs/config.template.yaml` to a new file, for example, `configs/my_experiment.yaml`.

3. **Modify the configuration:**
   Open `configs/my_experiment.yaml` and adjust the parameters for your experiment.
...
* `scripts/main_1data_hyperopt.py`: Script for hyperparameter optimization specifically tailored for single-dataset scenarios.
...
* `scripts/benchmark.py`: General script for performance benchmarking of PINN models.
* `scripts/benchmark_30min.py`: A specialized benchmarking script configured for a 30-minute execution duration.
...
* `scripts/generate_unified_dataset.py`: Creates a harmonized and unified dataset suitable for training and evaluation.
...
<br><sub>Last edited: 2025-12-09 14:31:08</sub>
