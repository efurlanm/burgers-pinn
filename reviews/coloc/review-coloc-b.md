...
* **PDE Loss Weight ($\\lambda_{pde}$):** 1.0 (changed to 10.0 for Run 7)
...
3. **Epochs and Runs for Testing:**
   ...
   * `num_runs` was temporarily set to `1` for the first run, and then to `7` with an `if i == 6:` condition for the seventh run.
   * For Run 7, `lambda_pde_weight` was temporarily set to `10.0` (from `1.0`).
...
| 10  | 1.000000e-02  | 0.7699            | 1.296202e-04   | 14.58            | 308.34                 | 243.59            |                       |
...
* **Adam Duration (s):** For the first run with the new configuration, the Adam optimization phase took 52.33 seconds. For the seventh run, it took 52.74 seconds. These durations are significantly less than the original 305-308 seconds, which was due to the temporary reduction of `adam_epochs` to 100 for testing purposes. For the full experiment, this duration will revert to the expected range.
* **L-BFGS-B Duration (s):** For the first run, the L-BFGS-B optimization phase took 628.26 seconds. For the seventh run, it took 6.21 seconds. This significant variation highlights the sensitivity of L-BFGS-B to the initial conditions provided by the Adam optimizer and the specific hyperparameters.
* **Overall Performance:** The initial test runs with reduced epochs and `num_runs = 1` (for run 1) and `num_runs = 7` with `if i == 6` (for run 7) completed much faster, confirming the code modifications are functional. The full experiment will provide a more accurate picture of the performance with the new collocation point strategy and adjusted PDE loss weight.
...
2. **Investigate $\\nu$ Sensitivity:** The high relative error despite low total loss suggests that the loss function might not be sufficiently sensitive to the $\\nu$ parameter.
   * **Action:** Experiment with different weighting strategies for $\\lambda_{data}\\$ and $\\lambda_{pde}\\$ . The current results for run 7 suggest that simply increasing $\\lambda_{pde}\\$ might not be effective. Further investigation into the optimal balance between data and PDE loss is needed.
...
<br><sub>Last edited: 2025-08-23 19:37:06</sub>
