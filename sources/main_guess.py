...
class PINN_Burgers2D:
    ...
    def reset_variables(self, initial_nu_value):
        """
        Re-initializes all trainable variables and optimizers for a new training run.
        """
        # Initialize the neural network weights and biases
        self.weights, self.biases = self.initialize_neural_network(self.layers)

        # Discoverable parameter: log of kinematic viscosity for stability
        self.log_nu_pinn = tf.Variable(tf.math.log(tf.constant(initial_nu_value, dtype=tf.float32)), dtype=tf.float32,
                                       name="log_nu_pinn")
        ...
        # Define Adam optimizer training operation for full loss
    ...
    def train(self, epochs_adam, epochs_data_only, X_np, Y_np, u_true_list, v_true_list, t_max, grid_points_x, grid_points_y):
        """
        Trains the PINN for multiple initial nu candidates and collects results.
        """
        num_candidates = 10
        nu_candidates = np.random.uniform(0.01, 0.5, num_candidates)
        
        all_candidate_results = []

        print(f"Starting full training for {num_candidates} random nu candidates...")

        for i, nu_candidate in enumerate(nu_candidates):
            print(f"\n--- Training Candidate {i+1}/{num_candidates}: nu_candidate = {nu_candidate:.6f} ---")
            
            # Reset variables for each new candidate
            self.reset_variables(nu_candidate)
            ...
            # --- Collect Results for Current Candidate ---
            final_nu = np.exp(self.session.run(self.log_nu_pinn))
            relative_error = abs(final_nu - self.true_nu) / self.true_nu * 100

            X_plot_flat = X_np.flatten()[:, None]
            Y_plot_flat = Y_np.flatten()[:, None]
            T_plot_flat = np.full_like(X_plot_flat, t_max)

            u_pinn_pred_flat, v_pinn_pred_flat = self.session.run(
            ...
            all_candidate_results.append({
                'nu_candidate': nu_candidate,
                'final_discovered_nu': final_nu,
                'relative_error': relative_error,
                'total_mse': total_mse,
                'duration_data_only': duration_data_only,
                'duration_adam': adam_duration,
                'duration_lbfgs': lbfgs_duration
            })
            print(f"  Final Discovered nu: {final_nu:.6f}, Relative Error: {relative_error:.2f}%, Total MSE: {total_mse:.6e}")
...
if __name__ == "__main__":
    ...
    # --- Model Training ---
    all_candidate_results = pinn.train(
        epochs_adam=adam_epochs,
        epochs_data_only=epochs_data_only,
        X_np=X_np,
        Y_np=Y_np,
    ...
    # --- Results and Saving ---
    print("\n--- Final Results for Each Initial Guess Candidate ---")
    print("{:<20} {:<25} {:<20} {:<20}".format("Candidate nu", "Final Discovered nu", "Relative Error (%)", "Total MSE"))
    print("-" * 85)
    for res in all_candidate_results:
        print("{:<20.6f} {:<25.6f} {:<20.2f} {:<20.6e}".format(
            res['nu_candidate'],
            res['final_discovered_nu'],
            res['relative_error'],
            res['total_mse']
        ))
    print("-" * 85)
    ...
    # Save all candidate results to a single NPZ file
    output_filename = f"guess_results_seed_{seed_value}.npz"
    output_file = os.path.join(results_dir, output_filename)
    
    # Convert list of dicts to a dictionary of lists for easier saving
    # This assumes all dicts in all_candidate_results have the same keys
    if all_candidate_results:
        keys = all_candidate_results[0].keys()
        data_to_save = {key: [d[key] for d in all_candidate_results] for key in keys}
        np.savez(output_file, **data_to_save)
        print(f"All candidate results saved to {output_file}")
    else:
        print("No candidate results to save.")

    overall_duration = time.time() - overall_start_time
    print("\n--- Overall HPC Performance Metrics ---")
