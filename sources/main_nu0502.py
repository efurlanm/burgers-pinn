...
if __name__ == "__main__":
    # --- Experiment Setup ---
    true_kinematic_viscosity_values = [0.05, 0.02]
    learning_rates_to_test = [1e-05, 1e-04, 1e-03, 1e-02]
    
    # --- Epochs for Testing (Reduced) ---
    adam_epochs_test = 10
    epochs_data_only_test = 100

    # --- Epochs for Full Run ---
    adam_epochs_full = 2000
    epochs_data_only_full = 10000

    # Set to full epochs for now
    adam_epochs = adam_epochs_full
    epochs_data_only = epochs_data_only_full
    ...
    for true_kinematic_viscosity in true_kinematic_viscosity_values:
        # --- Data Preparation (do this once per true_nu) ---
        ...
        for learning_rate in learning_rates_to_test:
            run_counter += 1
            print(f"--- Starting Run {run_counter} (True nu: {true_kinematic_viscosity}, Learning Rate: {learning_rate:.6e}) ---")
            ...
            final_loss, final_loss_data, final_loss_pde = pinn.session.run(
                [pinn.loss_op, pinn.loss_data_op, pinn.loss_pde_op])

            # Print results in a parsable format (CSV-like line)
            print(f"RESULT_NU0502_RUN,{run_counter},{true_kinematic_viscosity},{learning_rate:.6e},{final_nu:.6f},{relative_error:.4f},{final_loss:.6e},{final_loss_data:.6e},{final_loss_pde:.6e},{duration_data_only:.2f},{adam_duration:.2f},{lbfgs_duration:.2f}")
            
            print(f"Run {run_counter} Finished. Discovered nu: {final_nu:.6f}, Relative Error: {relative_error:.4f}")
            pinn.session.close()