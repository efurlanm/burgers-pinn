...
if __name__ == "__main__":
    # --- Experiment Setup ---
    learning_rates_to_test = np.logspace(np.log10(1e-5), np.log10(1e-2), 10)
    num_runs = 10 # Set to 10 for full experiment
    # results will be printed to stdout and parsed by the shell script
    
    # --- Epochs for Full Experiment ---
    adam_epochs = 2000
    ...
    for i in range(num_runs):
        if i == 9: # Only run the tenth iteration (i=9)
        ...
        learning_rate = learning_rates_to_test[i]
        ...
        print(f"RESULT_RUN,{i+1},{learning_rate:.6e},{final_nu:.6f},{relative_error:.4f},{final_loss:.6e},{duration_data_only:.2f},{adam_duration:.2f},{lbfgs_duration:.2f}")
