...
class PINN_Burgers2D:
    ...
    def train(self, epochs_adam, epochs_data_only):
        ...
        """
        # --- Find Best Initial Nu ---
        best_initial_nu = self.find_best_initial_nu([0.01, 0.05, 0.1, 0.5])
        print(f"Best initial nu found: {best_initial_nu}")
        self.session.run(tf_v1.assign(self.log_nu_pinn, tf.math.log(best_initial_nu)))
...
if __name__ == "__main__":
    ...
    for i in range(num_runs):
        ...
        # Generate a random learning rate (logarithmic scale)
        learning_rate = 10**-np.random.uniform(2, 5) # Between 1e-5 and 1e-2
        ...
        # --- Model Training ---
        pinn.train(epochs_adam=adam_epochs, epochs_data_only=epochs_data_only)
        ...
        results.append((learning_rate, final_nu, relative_error))
        print(f"Run {i+1} Finished. Discovered nu: {final_nu:.6f}, Relative Error: {relative_error:.4f}")
        pinn.session.close()

    # --- Print Final Results Table ---
    print("\n" + "="*50)
    print(" " * 10 + "Learning Rate Experiment Results")
    print("="*50)
    print(f"{ 'Run':<5} | { 'Learning Rate':<18} | { 'Discovered nu':<15} | { 'Relative Error':<15}")
    print("-"*50)
    for i, (lr, nu, err) in enumerate(results):
        print(f"{i+1:<5} | {lr:<18.6e} | {nu:<15.6f} | {err:<15.4f}")
    print("="*50)
