...
RESULTS_FILE="results/shape_ensemble_results_nu_0.05.csv"
echo "seed,discovered_nu,mse_u,mse_v,total_mse" > $RESULTS_FILE

# Loop through seeds and run experiment
for i in {1..3}
do
    echo "Running main_shape.py with seed $i"
    # Run python script and capture output
    output=$(python src/main_shape.py --seed $i)
    ...
    echo "$output" > logs/main_shape_nu_0.05_seed_$i.txt
done
