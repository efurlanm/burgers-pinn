...
"""
Main execution script for Hyperparameter Optimization (HPO) of the parametric
PINN surrogate model.

This script leverages the `hyperopt` library to search for the best set of
hyperparameters for the PINN model defined in `pinn_model.py`.

The workflow is as follows:
1.  Defines a search space for hyperparameters.
2.  The `objective_function` orchestrates the HPO trial:
    a. Calls `run_training_stage` to train the PINN (Stage 1).
    b. The trained model is then evaluated on a fixed, independent validation set.
3.  `hyperopt.fmin` drives the search process.
4.  The script can also be run in a single-run mode to test a specific set of
    hyperparameters.
"""
...
def run_training_stage(params):
    ...
    training_datasets = []
    print(f"Generating {num_datasets_for_generalization} datasets for generalization training...")
    
    X_data_list_s1, Y_data_list_s1, T_data_list_s1 = [], [], []
    for i in range(num_datasets_for_generalization):
...
def objective_function(params):
    ...
    results_dir = params.get('results_dir', 'results/hopt')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--trials_file', type=str, default='hyperopt_trials.pkl', help='File to save and load hyperopt trials.')
    # Add other arguments for single-run mode
    # ... (all previous single-run args) ...
    args = parser.parse_args()

    if args.optimize:
        space = {
            'seed': hp.choice('seed', [42, 97, 123]),
            'neurons': hp.choice('neurons', [40, 50, 60]),
            'layers': hp.choice('layers', [4, 5]),
            'learning_rate': hp.loguniform('learning_rate', np.log(1e-4), np.log(1e-3)),
            'adam_epochs_stage1': hp.choice('adam_epochs_stage1', [5000, 8000]),
            ...
            'epochs_inverse_adam_pretrain': hp.choice('epochs_inverse_adam_pretrain', [1000, 2000]),
            'num_datasets_gene': hp.choice('num_datasets_gene', [10, 15]),
            'noise_level': hp.uniform('noise_level', 0.0, 0.04)
        }
        ...
        best = fmin(
            ...
            space=space,
            algo=tpe.suggest,
            max_evals=len(trials.trials) + 5, # Run 5 new trials
            trials=trials
        )
        ...
        print("Running a single experiment with default/provided parameters.")
        # Reconstruct the params dict from args for a single run
        # This part needs to be filled out if single runs are desired
        default_params = {
             'seed': 42, 'neurons': 50, 'layers': 4, 'learning_rate': 0.00023,
             'adam_epochs_stage1': 5000, 'epochs_data_only_stage1': 500,
             'num_pde_points_stage1': 20000, 'epochs_inverse_adam_pretrain': 2000,
             'num_datasets_gene': 15, 'noise_level': 0.04
        }
        objective_function(default_params)
