...
class PINN_Burgers2D(tf.keras.Model):
    ...
    def compute_pde_residual(self, x, y, t, nu):
        with tf.GradientTape(persistent=True) as outer_tape:
            outer_tape.watch([x, y, t])
            with tf.GradientTape(persistent=True) as inner_tape:
                inner_tape.watch([x, y, t])
                u, v = self.predict_velocity(x, y, t, nu)
            u_x, u_y, u_t = inner_tape.gradient(u, x), inner_tape.gradient(u, y), inner_tape.gradient(u, t)
            v_x, v_y, v_t = inner_tape.gradient(v, x), inner_tape.gradient(v, y), inner_tape.gradient(v, t)
        u_xx, u_yy = outer_tape.gradient(u_x, x), outer_tape.gradient(u_y, y)
        v_xx, v_yy = outer_tape.gradient(v_x, x), outer_tape.gradient(v_y, y)
        del inner_tape
        del outer_tape
